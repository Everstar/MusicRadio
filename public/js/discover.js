...
var svg_background_color_online = '#0288D1',
  ...
  var songList = ['纯音乐', 'bilibili镇站之宝', '耳机福利', '欧美循环单曲'];
  var operationList = ['CreatePlayList', 'UpdatePlayList', 'LikePlayList', 'CommentPlayList'];

function createEvent(){

  var event = {
    'id': Math.random() * 100000,
    'type': operationList[parseInt(Math.random() * 4)] ,
...
function drawEvent(data, svg_area) {
    ...
    switch(data.type){
      case "CreatePlayList":
        label_text = data.user.capitalize() + " 创建 乐单 " + data.repo_name;
        edit_color = '#B2DFDB';
      break;
      case "UpdatePlayList":
        label_text = data.user.capitalize() + " 更新 乐单 " + data.repo_name;
        edit_color = '#C6FF00';
        ring_anim_duration = 10000;
        ring_radius = 600;
      break;
      case "LikePlayList":
        label_text = data.user.capitalize() + " 喜爱 乐单 " + data.repo_name;
        edit_color = '#FFEB3B';
      break;
      case "CommentPlayList":
        label_text = data.user.capitalize() + " 评论 乐单 " + data.repo_name;
        edit_color = '#FF5722';
      break;
    }
    var csize = size;
